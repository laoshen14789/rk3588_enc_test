...
#define mpp_err(format, arg...) { \
    printf("%s,%d:" format "\n", __FILE__, __LINE__, ##arg); \
}

#define mpp_log(format, arg...) { \
    printf("%s,%d:" format "\n", __FILE__, __LINE__, ##arg); \
}
...
MPP_RET test_ctx_init(MpiEncMultiCtxInfo *info)
{
    ...
    if (cmd->file_input) {
        if (!strncmp(cmd->file_input, "/dev/video", 10)) {
            ...
            p->cam_ctx = camera_source_init(cmd->file_input, 10, p->width, p->height, p->fmt);
...
MPP_RET test_mpp_enc_cfg_setup(MpiEncMultiCtxInfo *info)
{
    ...
    if (p->split_mode) {
        mpp_log("%p split mode %d arg %d out %d\n", ctx,
    ...
    while (!p->pkt_eos) {
        ...
        if (p->fp_input) {
            ...
            if (p->cam_ctx == NULL) {
                ...
                printf("camera_source_get_frame%lld\n", mpp_time());
                ...
                mpp_assert(cam_buf);
                printf("camera_frame_to_buf%lld\n", mpp_time());
            }
        }
        printf("mpp_frame_init%lld\n", mpp_time());
        ...
        printf("mpp_meta_set_buffer%lld\n", mpp_time());
        ...
        do {
            ...
            if (packet) {
                ...
                if (p->pkt_eos) {
                    mpp_log("chn %d found last packet\n", chn);
